...
Multi_Process::safety_monitor_build(const string& name)
{
	...
	if (!CreateProcess(
				...
				//					   CREATE_NEW_PROCESS_GROUP | CREATE_NEW_CONSOLE,// or  choose  CREATE_NEW_CONSOLE
				NULL,// Use parent's environment block
				NULL,// Use parent's starting directory
				&si, // Pointer to STARTUPINFO structure
				&pi) // Pointer to PROCESS_INFORMATION structure
	...
	//	EnumWindowsData data = {process_id,NULL};
	//	Sleep(1500);
	//	EnumWindows(EnumWindowsProc,(LPARAM)&data);
	//
	//    if (data.hwnd != NULL) {
	//        // 找到对应进程的窗口句柄，可以进行操作
	//        std::cout << "Found window handle: " << data.hwnd << std::endl;
	//        // 在这里可以对窗口进行操作，例如最小化
	//        ShowWindow(data.hwnd, SW_HIDE);
	//    } else {
	//        std::cout << "Window not found for process ID: " << process_id << std::endl;
	//    }
...
int Multi_Process::data_monitor(const string& name, HANDLE& handle_read)
{
	...
	bool ret = CreateProcess(NULL, (char*)name.data(), NULL, NULL, TRUE, CREATE_NEW_CONSOLE | CREATE_NEW_PROCESS_GROUP,
							 NULL, NULL, &si, &pi);
	if (!ret)
	{
